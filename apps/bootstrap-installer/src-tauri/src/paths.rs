...
//!   macOS:   ~/.hermeszooid
//!   Linux:   ~/.hermeszooid  (override via $HERMESZOOID_HOME)
//!
//! NOTE (macOS): Python's get_hermes_home(), scripts/install.sh, and the
//! Electron desktop's resolveHermesHome() ALL use ~/.hermeszooid on macOS — there
...
/// Returns the canonical Hermes home directory, respecting $HERMESZOOID_HOME if set.
pub fn hermes_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("HERMESZOOID_HOME") {
    ...
    {
        // %LOCALAPPDATA%\hermeszooid — matches scripts/install.ps1's $HermesHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("hermeszooid");
        }
    }

    // macOS + Linux + fallback: ~/.hermeszooid (matches Python get_hermes_home(),
    // install.sh, and the Electron desktop's resolveHermesHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".hermeszooid");
    }

    // Last resort — current dir, almost certainly wrong but at least
    // doesn't panic.
    PathBuf::from(".hermeszooid")
}
...
/// HERMESZOOID_HOME so it survives repo checkout deletion (unlike anything under
/// hermes-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\hermeszooid\hermes-setup.exe`; on other
...
///
/// Lives directly under HERMESZOOID_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves HERMESZOOID_HOME identically and pins it into
...
pub fn copy_self_to_hermes_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to HERMESZOOID_HOME");
    Ok(())
}
...
/// Initializes tracing to bootstrap-installer.log under HERMESZOOID_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[hermeszooid-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("HERMESZOOID_BOOTSTRAP_LOG")
