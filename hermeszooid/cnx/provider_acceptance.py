...
from .dispatcher_process import HermesZooidDispatcherProcess
...
class LiveProviderAcceptance:
    ...
    def child_env(self) -> dict[str, str]:
        process = HermesZooidDispatcherProcess(self.executor, state_dir=self.plan.state_dir)
    ...
    def dispatch_command(self, prepared: PreparedAcceptance) -> list[str]:
        return [
            sys.executable,
            "-m",
            "hermeszooid.cnx.dispatcher_child",
            "--mode",
            "dispatch-task",
            "--db",
    ...
    def reconcile(self, prepared: PreparedAcceptance) -> AcceptanceResult:
        ...
        with self.open_store() as store:
            ...
            store.record_evidence(
                ...
                value=actual,
                criterion_index=1,
                metadata={"path": str(self.plan.artifact_path), "verified_by": "hermeszooid-parent"},
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="python -m hermeszooid.cnx.provider_acceptance")
